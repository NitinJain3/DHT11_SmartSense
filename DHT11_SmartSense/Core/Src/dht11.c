...
#define TEMP_THRESHOLD       30   /* Adjust as needed */
#define HUMIDITY_THRESHOLD   70   /* Adjust as needed */
...
uint8_t DHT11_CheckResponse(void) {
	...
	if (response) {
		...
		/*printf("DHT11 Response OK - DHT11 Data pin available\r\n");*/
	} else {
		DEBUG_ERROR("DHT11 Response Failed (Initial LOW or HIGH missing)\n");
		printf("DHT11 Response Failed - DHT11 Data pin not available\r\n");
		HAL_Delay(1000);
	}
	return response;
}
...
void ReadAndDisplayDHT11(void) {
	static uint8_t last_exceeded_state = 0; /* 0 = normal, 1 = exceeded*/
	uint8_t rh_int;
	uint8_t rh_dec;
	uint8_t t_int;
	uint8_t t_dec;
...
/**
 * @brief Checks whether the temperature or humidity exceeds predefined thresholds.
 *
 * This helper function compares the current temperature and humidity values
 * against the global constants TEMP_THRESHOLD and HUMIDITY_THRESHOLD.
 *
 * @param temp     Temperature integer value from DHT11 sensor.
 * @param humidity Humidity integer value from DHT11 sensor.
 * @return uint8_t Returns 1 if threshold is exceeded, else 0.
 */
uint8_t is_threshold_exceeded(uint8_t temp, uint8_t humidity) {
	return (temp >= TEMP_THRESHOLD || humidity >= HUMIDITY_THRESHOLD);
}
